...
def get_wavelength(h5,wd):
	...
	wavelength.append(2465)
	return(wavelength)

def dt_fusion(brdf_rfl):
	...
	chm = gdal.Open(hfile)
	height= chm.ReadAsArray()
	for i in list(range(0,height.shape[0])):
		row= height[i,:]
		for j in list (range(0,height.shape[1])):
			if row[j]>9: row[j]=0
		height[i,:]=row
	#normalizing heigh
	h_data =  height.reshape(-1)
	normh= (h_data-min(h_data))/(max(h_data)-min(h_data))
	hnd =  normh.reshape((data.shape[0], data.shape[1]))
	#data fusion
	nh = hnd[:,:,newaxis]
	alld = np.concatenate((data,nh),axis=2)
	return(alld)
...
def get_FC(brdf_rfl,U):
	...
	amaps = am.map(alld,U,normalize=False) 
	#diaplay
...
data = dt_fusion(sig_files[0])
for x in list(range(1,10)):
	dt = dt_fusion(sig_files[x])
	data = np.concatenate((data,dt), axis = 0)
print(data.shape)
wd = '/N/project/SRER/Unmixing/BRDFHeight/10TSig/'
